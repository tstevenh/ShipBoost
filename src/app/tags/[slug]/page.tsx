...
export async function generateMetadata(
  ...
  const canonical = `${getEnv().NEXT_PUBLIC_APP_URL}/tags/${slug}`;
...
export default async function TagPage(context: RouteContext) {
  const { slug } = await context.params;
  const page = await getCachedBestTagPage(slug);
  ...
  const env = getEnv();
  const canonical = `${env.NEXT_PUBLIC_APP_URL}/tags/${slug}`;
  const relatedCategoryMap = new Map<
    string,
        ...
        <ShowcaseLayout>
          <div className="space-y-10">
            <nav className="flex items-center gap-2 text-xs font-bold text-muted-foreground/60 tracking-widest">
              ...
              <ChevronRight size={12} />
              <span className="text-primary font-black">#{page.tag.name}</span>
            </nav>
            ...
            <div className="max-w-4xl">
              ...
              <div className="flex flex-wrap items-center gap-6">
                ...
                >
                  <Trophy size={16} />
                  See products tagged {page.tag.name}
                </Link>
                <Link
                  href="/tags"
                  className="flex items-center gap-2 text-sm font-black text-primary hover:opacity-80 transition-opacity"
            ...
            <section className="rounded-[2rem] border border-border bg-card p-8 shadow-sm">
              <h2 className="text-3xl font-black tracking-tight text-foreground">
                What teams usually mean by {page.tag.name}
              </h2>
              <div className="mt-4 space-y-3 text-base font-medium leading-relaxed text-muted-foreground/80">
                ...
                {relatedTagNames.length > 0 ? (
                  <p>
                    It also overlaps with tags such as {relatedTagNames.join(", ")},
                    which is why this page works best as a refinement layer rather
